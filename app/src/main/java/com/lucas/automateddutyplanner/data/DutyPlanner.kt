...
fun dutyPlanningMethodByDate(
    daS: List<DutyAssistant>,
    ...
    for (day in daysOfMonth) {
        val isWeekend =
            day.dayOfWeek == kotlinx.datetime.DayOfWeek.SATURDAY || day.dayOfWeek == kotlinx.datetime.DayOfWeek.SUNDAY || day in publicHolidays
        dutyAssistants = if (isWeekend) {
            // weekend set priorities
            dutyAssistants.map {
                if (it.constraints.contains(Constraint.EXCUSE_STAY_IN) || it.constraints.contains(
                        Constraint.PREFER_WEEKEND
                    )
                ) {
                    it.priority += 1
                    it.isTouched = true
                ...
                if (day.dayOfWeek == kotlinx.datetime.DayOfWeek.SATURDAY && it.constraints.contains(
                        Constraint.PREFER_SATURDAY
                    )
                ) {
                    it.priority += 1
                    it.isTouched = true
                } else if (day.dayOfWeek == kotlinx.datetime.DayOfWeek.SUNDAY && it.constraints.contains(
                        Constraint.PREFER_SUNDAY
                    )
                ) {
                    it.priority += 1
                    it.isTouched = true
            ...
            dutyAssistants.map {
                if (it.constraints.contains(Constraint.PREFER_WEEKDAY)) {
                    it.priority += 1
                    it.isTouched = true
                } else if (day.dayOfWeek == kotlinx.datetime.DayOfWeek.MONDAY && it.constraints.contains(
                        Constraint.PREFER_MONDAY
                    )
                ) {
                    it.priority += 1
                    it.isTouched = true
                } else if (day.dayOfWeek == kotlinx.datetime.DayOfWeek.TUESDAY && it.constraints.contains(
                        Constraint.PREFER_TUESDAY
                    )
                ) {
                    it.priority += 1
                    it.isTouched = true
                } else if (day.dayOfWeek == kotlinx.datetime.DayOfWeek.WEDNESDAY && it.constraints.contains(
                        Constraint.PREFER_WEDNESDAY
                    )
                ) {
                    it.priority += 1
                    it.isTouched = true
                } else if (day.dayOfWeek == kotlinx.datetime.DayOfWeek.THURSDAY && it.constraints.contains(
                        Constraint.PREFER_THURSDAY
                    )
                ) {
                    it.priority += 1
                    it.isTouched = true
                } else if (day.dayOfWeek == kotlinx.datetime.DayOfWeek.FRIDAY && it.constraints.contains(
                        Constraint.PREFER_FRIDAY
                    )
                ) {
                    it.priority += 1
                    it.isTouched = true
        ...
        try {
            if (isReserve) {
                ...
                val dutyGuyIdx = dutyAssistants.indexOf(dutyAssistants.find {
                    (it.assignedReserve.isEmpty() || abs(
                        day.toEpochDays() - it.assignedReserve.last().toEpochDays()
                    ) > 1) && Constraint.EXCUSE_STAY_IN !in it.constraints && day !in it.unableDates && !it.assignedDates.any { a -> a == day }
                })
                if (dutyGuyIdx != -1) {
                ...
                val dutyGuyIdx = dutyAssistants.indexOf(dutyAssistants.find {
                    (it.assignedDates.isEmpty() || abs(
                        day.toEpochDays() - it.assignedDates.last().toEpochDays()
                    ) > 1) && day !in it.unableDates
                })
                if (dutyGuyIdx != -1) {
                    if (dutyAssistants[dutyGuyIdx].constraints.contains(Constraint.EXCUSE_STAY_IN)) {
                        ...
                        val second =
                            dutyAssistants.find {
                                Constraint.EXCUSE_STAY_IN !in it.constraints && (it.assignedDates.isEmpty() || abs(
                                    day.toEpochDays() - it.assignedDates.last().toEpochDays()
                                ) > 1) && day !in it.unableDates
                            }
                ...
                val dutyGuyIdx = dutyAssistants.indexOf(dutyAssistants.find {
                    (it.assignedDates.isEmpty() || abs(
                        day.toEpochDays() - it.assignedDates.last().toEpochDays()
                    ) > 1) && Constraint.EXCUSE_STAY_IN !in it.constraints && day !in it.unableDates
                })
                if (dutyGuyIdx != -1) {
