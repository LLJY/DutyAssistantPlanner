...
class MainViewModel @Inject constructor(application: Application) : AndroidViewModel(application) {
    private val _dutyAssistantList: MutableStateFlow<List<DutyAssistant>> =
    ...
    private var _publicHolidaysFlow: MutableStateFlow<List<LocalDate>> =
    ...
    private suspend fun _initialiseValues() {
        sharedPrefs =
            getApplication<Application>().getSharedPreferences("somekeylol", Context.MODE_PRIVATE)
        ...
        if (!publicHolsRawStr.isNullOrEmpty()) {
            ...
            if (_publicHolidays.any { it.year != selectedYear }) {
                ...
                sharedPrefs.edit().putString("publicHolidays", Json.encodeToString(_publicHolidays))
                    .apply()
            }
    ...
    private suspend fun getSavedList() {
    ...
    private suspend fun savePlannedDuty() = withContext(Dispatchers.IO) {
        sharedPrefs.edit().putString("plannedList", Json.encodeToString(_dutyPlannedList)).apply()
        sharedPrefs.edit().putString("reserveList", Json.encodeToString(dutyReserveList)).apply()
    }

    private suspend fun saveModifiedList() = withContext(Dispatchers.IO) {
        sharedPrefs.edit().putString("daList", Json.encodeToString(_dutyAssistants)).apply()
        sharedPrefs.edit().putInt("month", selectedMonth).apply()
    }
    ...
    suspend fun planDuty(isReserve: Boolean = false) {
        ...
        val results = dutyPlanningMethodByDate(
            _dutyAssistants,
    ...
    suspend fun createCsvFromDuty(directory: File, reserve: Boolean): File =
    ...
    suspend fun importJsonToDaConfig(inputStream: InputStream): Boolean =
        withContext(Dispatchers.IO) {
            try {
    ...
    suspend fun importIcsToLocalDate(inputStream: InputStream): Boolean =
        withContext(Dispatchers.IO) {
            val format = LocalDate.Formats.ISO_BASIC
            val returnList: MutableList<LocalDate> = mutableListOf()
            try {
                val dateLine = inputStream.bufferedReader().use { it.readLines() }
                    .filter { it.contains("DTSTART;") }
                dateLine.forEach {
    ...
    private fun getDaysInMonth(year: Int, month: Int): List<LocalDate> {
        ...
        val daysInMonth =
            firstDayOfMonth.month.length((year % 4 == 0 && year % 100 != 0) || (year % 400 == 0))
